...
function AccordionRow({
    ...
    substansi: PenugasanInfo;
    reviewers: Reviewer[];
    onEdit: (jenis: number) => void;
    onDelete: (jenis: number) => void;
}) {
    const hasAny = administrasi.id_penugasan || substansi.id_penugasan;
    ...
    const items = [
        { label: 'Administrasi', jenis: 1, data: administrasi },
        { label: 'Substansi',    jenis: 2, data: substansi    },
    ];
        ...
        <tr>
            <td colSpan={7} className="bg-gray-50 border-b border-gray-100">
                <div className="px-6 py-3 space-y-3">
                    {items.map(({ label, jenis, data }, gi) => {
                        if (!data.id_penugasan) return null;
                        const reviewer = reviewers.find(r => r.id === data.id_reviewer);
                        const sc = STATUS_PENUGASAN[data.status_review ?? ''] ?? { text: 'text-gray-500', dot: 'bg-gray-400' };
                        ...
                        return (
                            <div key={jenis}>
                                ...
                                <div className="flex items-center justify-between py-1.5">
                                    <div>
                                        <p className="text-xs font-medium text-gray-700">{reviewer?.name ?? '—'}</p>
                                        <p className="text-xs text-gray-400">{reviewer?.email ?? '—'}</p>
                                    </div>
                                    <div className="flex items-center gap-5">
                                        <div className="text-right">
                                            <p className="text-xs text-gray-400">Batas</p>
                                            <p className="text-xs text-gray-600">{data.batas_waktu ?? '—'}</p>
                                        </div>
...
function Modal({
    ...
    // Sync form saat editData berubah
    ...
    // ── Logika validasi ──────────────────────────────────────────────────────
    const lolosAdministrasi   = isLolosAdministrasi(penelitian.administrasi?.hasil);
    const administrasiSudahAda = !!penelitian.administrasi?.id_penugasan;

    const administrasiBlocked = form.id_jenis_penugasan === '1' && administrasiSudahAda && mode === 'create';
    const substansiBlocked    = form.id_jenis_penugasan === '2' && !lolosAdministrasi;
                    ...
                    <div>
                            ...
                            {[
                                {
                                    id: '1',
                                    label: 'Administrasi',
                                    hint: administrasiSudahAda && mode === 'create'
                                        ? 'Sudah ditugaskan'
                                        : 'Maks. 1 reviewer',
                                },
                                {
                                ...
                                const isLocked =
                                    (j.id === '1' && administrasiSudahAda && mode === 'create') ||
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <span className={`block text-[10px] mt-0.5 ${
                                            isLocked ? 'text-gray-300' : isSelected ? 'text-green-100' : 'text-gray-400'
                                        }`}>
                                            {j.hint}
                                        </span>
                                    </button>
                        ...
                        {form.id_jenis_penugasan === '2' && !lolosAdministrasi && (
                            <div className="mt-2 flex items-start gap-1.5 text-xs text-amber-600">
                                <AlertCircle className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                                <span>
                                    Penugasan substansi hanya bisa dilakukan setelah penelitian dinyatakan{' '}
                                    <strong>Lolos</strong> administrasi.
                                </span>
                            </div>
                        )}
                    </div>
...
export default function PenugasanReviewIndex() {
    ...
    const filtered = useMemo(() =>
        penelitian.filter(p => {
            ...
            const matchSearch = !q || p.title.toLowerCase().includes(q) || (p.user?.name ?? '').toLowerCase().includes(q);
            ...
            const matchAdmin  = !filterAdministrasi ||
                (filterAdministrasi === 'belum'
                    ? !p.administrasi.hasil
                    : p.administrasi.hasil === filterAdministrasi);
    ...
    // ── Summary counts ───────────────────────────────────────────────────────
    const totalBelumDitugaskan = penelitian.filter(p => !p.administrasi.id_penugasan).length;
    const totalSudahDitugaskan = penelitian.filter(p => !!p.administrasi.id_penugasan).length;
    const totalLolos           = penelitian.filter(p => p.administrasi.hasil === 'Lolos').length;
    ...
    const openEdit = (p: Penelitian, jenis: number) => {
        const data = jenis === 1 ? p.administrasi : p.substansi;
        setEditTarget({
    ...
    const handleDelete = (p: Penelitian, jenis: number) => {
        const data = jenis === 1 ? p.administrasi : p.substansi;
        if (!data.id_penugasan) return;
        if (!confirm('Hapus penugasan ini?')) return;
        router.delete(`/admin/pt-penelitian/penugasan-review/${data.id_penugasan}`, {
            preserveScroll: true,
        });
    };
                    ...
                    <div className="overflow-hidden border border-gray-200 bg-white">
                        <div className="px-6 py-4 border-b border-gray-100">
                            ...
                            <p className="text-xs text-gray-500 mt-0.5">
                                Klik baris untuk melihat penugasan. Substansi hanya bisa ditugaskan jika administrasi sudah{' '}
                                <strong>Lolos</strong>.
                            </p>
                        </div>
                                        ...
                                        return (
                                            <>
                                                ...
                                                >
                                                    ...
                                                    {/* Status administrasi */}
                                                    <td className="px-6 py-4">
                                                        <AdministrasiBadge status={p.administrasi.hasil} />
                                                    </td>
                                                ...
                                                {isOpen && (
                                                    <AccordionRow
                                                        ...
                                                        reviewers={reviewers}
                                                        onEdit={(jenis) => openEdit(p, jenis)}
                                                        onDelete={(jenis) => handleDelete(p, jenis)}
                                                    />
                                                )}
                                            </>
                                        );
