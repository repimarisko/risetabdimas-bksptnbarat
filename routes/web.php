...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    Route::get('/forbidden', function () {
        ...
        return Inertia::render('errors/forbidden', ['redirectTo' => $dashboard]);
    })->name('active-role.forbidden');

    // Switch active role
    Route::post('settings/active-role', function (Request $request) {
        ...
        return redirect($dashboardMap[$role] ?? '/dashboard')
            ->with('success', 'Role aktif diperbarui.');
    ...
    // ── Dosen ─────────────────────────────────
    Route::middleware('active_role:dosen')->group(function () {
        Route::get('dashboard/dosen', DosenDashboardController::class)
            ->name('dashboard.dosen');
    });

    // ── Admin PT ──────────────────────────────
    Route::middleware('active_role:admin-pt')->group(function () {
    ...
    // ── Ketua LPPM ────────────────────────────
    Route::middleware('active_role:ketua-lppm')->group(function () {
        Route::get('dashboard/ketua-lppm', fn() => Inertia::render('dashboard/ketua-lppm'))
            ->name('dashboard.ketua-lppm');
    });

    // ── Reviewer ──────────────────────────────
    Route::middleware('active_role:reviewer')->group(function () {
    ...
    // Preview proposal (semua role aktif yang relevan)
    Route::middleware('active_role:dosen,ketua-lppm,reviewer,admin-pt,super-admin')
        ->get('/pt-penelitian/{ptPenelitian}/preview', [PtPenelitianController::class, 'preview'])
